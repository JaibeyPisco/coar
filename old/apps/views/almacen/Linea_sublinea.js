...
let Componente = {
    render: async (d) => {
        ...
        <div id="main">
                                ...
                                <div class="box-body">
                                    <div class="row">
                                        <div class="col-md-12" align="right" style="margin-bottom:10px;">
                                            <button type="button" class="btn btn-sm btn-warning" name="nuevo"><i class="fa fa-plus"></i> Nueva Línea</button>
                                        </div>
                                        <div class="col-md-12">
                                    ...
                                    <div class="row">
                                        <div class="col-md-12" style="margin-bottom:10px;">
                                            <div class="row">
                                                ...
                                                <div class="col-md-5">
                                                    <select name="id_linea" data-select="LINEA" class="form-control" /><select>
                                                </div>
                                                <div class="col-md-4" align="right" >
                                                    <button type="button" class="btn btn-sm btn-success" name="nuevo_sublinea"><i class="fa fa-plus"></i> Nueva Sublínea</button>
                                                </div>
                                        ...
                                        <div class="col-md-12">
                                            <div class="table-responsive">
                                                <table name="registros_sublinea" class="table table-striped" style="width:100%;"></table>
                                            </div>
                                        </div>
                                    </div>                                                   
                                </div>                        
            ...
            <!-- MODAL SAVE -->
            <div class="modal inmodal fade" name="modal-save_sublinea" data-backdrop="static"  role="dialog" aria-hidden="true">
                <div class="modal-dialog modal-sm">
                    <div class="modal-content">
                        <div class="modal-header">
                            <button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
                            <h4 name="save_sublinea" class="modal-title">Modal title</h4>
                        </div>
                        <form name="save_sublinea">
                            <div class="modal-body">
                                <div class="row">                                   
                                    <div class="col-md-12">
                                        <div class="form-group">
            ...
            <!-- MODAL DELETE -->
            <div class="modal inmodal fade" name="modal-delete_sublinea" data-backdrop="static" tabindex="-1" role="dialog" aria-hidden="true">
                <div class="modal-dialog modal-sm">
                    <div class="modal-content">
                        <div class="modal-header">
                            <button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">×</span><span class="sr-only">Close</span></button>
                            <h4 name="delete_sublinea" class="modal-title">Modal title</h4>
                        </div>
                        <form name="delete_sublinea">
                            <div class="row">
                                <div class="col-md-12" align="center">
                                    <i class="fad fa-trash-alt fa-4x"></i><br/>
                                </div>
    ...
    after_render: async () => {
        ...
        /**********************SUBLINEA ******************/
         ...
         /** SUBMIT SAVE */
         DOM.find('form[name="save_sublinea"]').validate({
            ...
            submitHandler: function() {
                Componente.submit_sublinea();
            }
          
        });

        /** SUBMIT DELETE */
        DOM.find('form[name="delete_sublinea"]').validate({
            submitHandler: function() {
                Componente.submit_sublinea();
            }
        });
        ...
        DOM.on('click', 'button[name="nuevo_sublinea"]', function(e) {
            e.stopImmediatePropagation();
            Componente.new_sublinea();
        });

        /* EDITAR */
        DOM.on('click', 'button[name="row-edit_sublinea"]', function(e) {
            e.stopImmediatePropagation();
            Componente.edit_sublinea($(this));
        });

        /* ELIMINAR */
        DOM.on('click', 'a[name="row-delete_sublinea"]', function(e) {
            e.stopImmediatePropagation();
            Componente.delete_sublinea($(this));
        });
        ...
        DOM.on('change', 'select[name="id_linea"]', function(e) {
            e.stopImmediatePropagation();
            Componente.table_sublinea.ajax.reload(null, false);
        });

        await Componente.select_linea();
        
        Componente.datatable();
        Componente.datatable_sublinea();
    ...
    select_linea: () =>
    {
        ...
        axios.get(BASE_API+'almacen/linea/get_select')
    ...
    datatable_sublinea: function() {

        this.table_sublinea = DOM.find('table[name="registros_sublinea"]').DataTable({
            ajax:   {
                url: BASE_API + 'almacen/sublinea',
                data: function(d)
                {
                    d.id_linea = DOM.find('select[name="id_linea"]').val();
                }
            },
            ...
            columns: [{
                    ...
                    render: function(data, type, row) {
                        var html = `
                            <div class="btn-group">
                                <button type="button" class="btn btn-default btn-sm" name="row-edit_sublinea">EDITAR</button>     
                                <button type="button" class="btn btn-default btn-sm dropdown-toggle" data-toggle="dropdown">
                                      <i class="fa fa-angle-down"></i>
                                </button>
                                <ul class="dropdown-menu dropdown-menu-left" role="menu">
                                    <li><a class="dropdown-item" name="row-delete_sublinea" href="javascript:"><i class="far fa-trash-alt"></i> Eliminar</a></li>
                                </ul>
                            </div>
                        `;
    ...
    new_sublinea: function() {

        let id_linea = DOM.find('select[name="id_linea"]').val();
       
        if(id_linea == '')
        {
            HELPER.notificacion('Debe seleccionar una línea', 'warning');
            return false;
        }
        ...
        DOM.find('h4[name="'+accion+'"]').text('Nueva Sublínea');

        /** DATA */
        HELPER.reset_form(form);

        this.id_sublinea = null;
    ...
    edit_sublinea: function(row) {
        
        let accion = 'save_sublinea';
        let form = DOM.find('form[name="save_sublinea"]');

        DOM.find('h4[name="'+accion+'"]').text('Editar Sublínea');
    ...
    delete_sublinea: function(row) {

        let accion = 'delete_sublinea';
        let form = DOM.find('form[name="'+accion+'"]');

        DOM.find('h4[name="'+accion+'"]').text('Eliminar Sublínea');
    ...
    submit_sublinea: function() {
        ...
        axios({
            method: 'post',
            url: BASE_API + 'almacen/sublinea/' + this.action_submit,
            data: formData
        })
        .then(function(response) { 
            Componente.table_sublinea.ajax.reload(null, false);
    ...
    new: function() {
        ...
        DOM.find('h4[name="'+accion+'"]').text('Nueva Línea');
    ...
    submit: function() {
        ...
        axios({
            method: 'post',
            url: BASE_API + 'almacen/linea/' + this.action_submit,
            data: formData
        })
        .then(function(response) { 
