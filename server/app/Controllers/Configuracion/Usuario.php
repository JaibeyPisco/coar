...
class Usuario extends BaseController
{
	...
	public function index()
	{
        ...
        $response = $this->Usuario_m->select('usuario.*')
            ...
            ->select('estudiante.apellidos_nombres as estudiante, estudiante.id as id_estudiante')
//            ->join('local l', 'l.id = usuario.id_local', 'left')
        ...
        //		$response = $this->Usuario_m->select('usuario.*, usuario.tipo_persona as nombre_tipo_personal_usuario')
//		->select("coalesce(l.descripcion, '') as local")
//		->select('rol.nombre as nombre_rol')
//		->select('cliente.razon_social as cliente')
//		->select('CONCAT(usuario.nombre, " ", usuario.apellido) as nombreCompleto')
//		->select('tipo_personal.id as id_tipo_personal, tipo_personal.nombre as nombre_tipo_personal')
//		->join('area l', 'l.id = usuario.id_local', 'left')
//		->join('rol', 'rol.id = usuario.id_rol', 'left')
//		->join('socio cliente', 'cliente.id = usuario.id_cliente', 'left')
//		->join("area", "area.id = usuario.id_local", "left")
//		->join("personal", "personal.id = usuario.id_personal", "left")
//		->join("tipo_personal", "tipo_personal.id = personal.id_tipo_personal", 'left')
////		->select("area.nombre as nombreLocalActual")
//		->where('usuario.id_empresa', ID_EMPRESA)
...
//            $db->transStart();
            ...
            if(isset($data_request["id_personal"]) && $data_request["id_personal"] != '')
            {
                ...
                if(isset($data_request["id_estudiante"]) && $data_request["id_estudiante"] != null)
                {
                    ...
                    $data_request["nombre"] = $estudiante->apellidos_nombres;
...
//        }
    ...
    public function getTipoPersonalPorId($idTipoPersonal){
		try {

			$db = \Config\Database::connect();
			$db->transStart();

			$response = $this->TipoPersonalModel
				->select("id, nombre, descripcion, id_usuario, estado")
				->where('id', $idTipoPersonal)
				->where('estado', 1)
				->first();
