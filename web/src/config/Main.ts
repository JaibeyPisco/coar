...
let Main = {
    ...
    calcular_tiempo: () => {
        ...
        if (Main.tiempo_inicial >= 3) {
            let menu = document.querySelector<HTMLDivElement>('#mensaje_lento');
    ...
    initialize: async () => {
        ...
        axios({
            ...
            .then(function (response) {
                ...
                // if (data.usuario.tipo_persona === 'CLIENTE') {
                //     /** VIEW APP SYSTEM*/
                //     Main.body('app_cliente', data);

                // } else 
                if (data.usuario.tipo_persona === 'ESTUDIANTE') {

                    Main.body('app_estudiante', data);
                }
                else {
                    /** VIEW APP SYSTEM*/
                    Main.body('app', data);
    ...
    body: async (tipo: 'autenticacion' | 'app' | 'app_estudiante', data_user = null) => {
            ...
            if (tipo != 'autenticacion') {
                ...
                switch (tipo) {
                    ...
                    case 'app':
                        ...
                        html_main = await import('../views/componentes/Main_app.ts');
                    ...
                    //
                    //     // DRAW BODY APP
                    //     html_main = await import('../views_estudiante/componentes/Main_app.js');
                    //     await $('#contenedor_principal').html(html_main.default.render(data_user));
                }
                ...
                Loader.requireFiles(rutas, async function () {
                    ...
                    // ENRUTADOR        
                    let modulo = await import('./Sammy_router.ts');
