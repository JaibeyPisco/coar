import  modalFotos from './index.html.ts';

 let eView: HTMLElement;

 interface Archivo {
     archivo: string;
     nombre: string;
 }
const show = async (id_incidencia: number, id_incidencia_monitoreo?: number) => {
...
const load = async (element: HTMLElement) => {
    ...
    const foundElement  = element.querySelector('div[name="modal-fotos"]') as HTMLElement | null;
    if (!foundElement) {
